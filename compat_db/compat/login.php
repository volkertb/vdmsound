<?php
  ...
  if (isset($user, $password) && !isset($error)) {
    ...
    if (AuthLogin($user, $password)) {
      ...
      $redirect = HtmlMakeURL($SCRIPT_NAME, Array('user' => $user, 'error' => ErrGetLastError(), 'redirect' => $redirect));
    }
...
<?php
  ...
  echo('<tr><td align="right">User ID:</td><td><input class="flat" type="text" size="16" name="user" value="' . $user . '" maxlength="12"></td></tr>');
  echo('<tr><td align="right">Password:</td><td><input class="flat" type="password" size="16" name="password" maxlength="48"></td></tr>');
  ...
  echo('<li>' . HtmlMakeLink('Forgot your password?', 'account.php', Array('action' => 'forgot', 'user' => $user)) );
